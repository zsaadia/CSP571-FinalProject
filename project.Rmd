...
packages <- c("tidyverse", "tidyquant", "caret", "randomForest", "pROC", "ggplot2", "xgboost", "corrplot")
...
auc_xgb_bal <- auc(roc_xgb_bal)

cat("Balanced Model - AUC:", round(auc_xgb_bal, 4), "\n")
...
6 month prediction:
```{r}
latest_row   <- df %>% slice_tail(n = 1)
latest_date  <- latest_row$date
threshold_6m <- optimal_threshold_6mo$threshold

latest_matrix_6mo <- model.matrix(~ Unemployment_Lag + CPI_Lag + 
                                    InterestRate_Lag + YieldSpread_Lag - 1,
                                  data = latest_row)
latest_prob_6mo <- as.numeric(predict(xgb_6mo, latest_matrix_6mo))
latest_pred_6mo <- ifelse(latest_prob_6mo > threshold_6m, "Yes", "No")

cat("REAL-TIME 6-MONTH FORECAST\n")
cat("Latest Data Date:", as.character(latest_date), "\n")
cat("Predicted probability (6 months ahead):", round(latest_prob_6mo, 4), "\n")
cat("Recession expected?", latest_pred_6mo, "(Threshold =", round(threshold_6m, 4), ")\n")


# Plot
plot_data_6mo <- tibble(
  Label = "6-Month Recession Probability",
  Probability = latest_prob_6mo,
  Threshold = threshold_6m
)

ggplot(plot_data_6mo, aes(Label, Probability)) +
  geom_col(fill = ifelse(latest_pred_6mo == "Yes", "red", "steelblue"), width = 0.5) +
  geom_hline(yintercept = threshold_6m, color = "darkgreen", linetype = "dashed", linewidth = 1) +
  annotate("text", x = 1, y = threshold_6m + 0.05,
           label = paste("Threshold =", round(threshold_6m, 2)),
           color = "darkgreen", size = 4, hjust = 0.5) +
  scale_y_continuous(limits = c(0, 1)) +
  labs(title = "Real-Time 6-Month Recession Forecast",
       subtitle = paste("Prediction Date:", as.character(latest_date)),
       y = "Predicted Probability", x = "") +
...
```
Citations: OpenAI. (2025)
This work was completed with assistance from ChatGPT
